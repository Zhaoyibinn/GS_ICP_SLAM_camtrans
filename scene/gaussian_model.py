...
class GaussianModel(nn.Module):
    ...
    #     """
    #     Convert a batch of rotation matrices to a batch of quaternions.
    #     Args:
    #     R (Tensor): A tensor of shape (n, 3, 3) representing rotation matrices.
    #     Returns:
    ...
    #     trace = torch.trace(R, dim1=1, dim2=2)
    #
    #     # Calculate the four components of the quaternion
    #     w = 0.5 * torch.sqrt(trace + 1)
    #     x = 0.5 * torch.sqrt(trace - 1 + R[:, 2, 2] - R[:, 0, 0]) / w
    #     y = 0.5 * torch.sqrt(trace - 1 - R[:, 2, 2] + R[:, 0, 0]) / w
    #     z = 0.5 * torch.sqrt(trace - 1 + R[:, 1, 1] - R[:, 0, 0]) / w
    #
    #     # Stack the components to form the quaternions
    #     quaternions = torch.stack((w, x, y, z), dim=2)
    #
    #     return quaternions
    ...
    def quaternion_inverse(self,q):
        """计算四元数的逆"""
        conj_q = self.quaternion_conjugate(q)
        norm_q = torch.norm(q, p=2)
        if norm_q == 0:
            raise ValueError("四元数的模不能为0")
        return conj_q / (norm_q ** 2)
    ...
    def __init__(self, sh_degree : int):
        ...
        self._camera_quaternion = torch.empty(0)
        self._camera_t = torch.empty(0)


    def import_camera_rt(self,q,t):
        self._camera_quaternion = copy.deepcopy(nn.Parameter(q.requires_grad_(True)))
    ...
    def training_camera_setup(self,training_args):
        l_camera = [{'params': [self._camera_quaternion], 'lr': 1e-4, "name": "camera_q"},
